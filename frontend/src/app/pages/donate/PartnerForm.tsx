...
import { Building2, ChevronRight, Search, Users, CreditCard, Lock, CheckCircle2, X } from "lucide-react";
    ...
    try {
      const res = await fetch(`${API}/api/donations/create`, {
        ...
        body: JSON.stringify({
          ...
          pan: form.panNumber,
          programName: Object.keys(selectedPrograms).length > 0 ? Object.keys(selectedPrograms).map(id => PROGRAMS.find(p => p.id === id)?.name).join(", ") : "CSR Sponsorship",
      ...
      const options = {
        ...
        name: "WOMBTO18 Foundation", description: Object.keys(selectedPrograms).length > 0 ? `CSR Sponsorship for ${Object.keys(selectedPrograms).length} program(s)` : "CSR Sponsorship",
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <PInput id="partner-pan" label="PAN Number" value={form.panNumber} onChange={(e: any) => set("panNumber", e.target.value.toUpperCase())} placeholder="ABCDE1234F" maxLength={10} style={{ textTransform: "uppercase" }} />
        </div>
        <div className="mt-4">
      ...
      <ProgramSelector
        ...
        sectionTitle="Sponsor Programs"
        sectionSubtitle="Choose multiple programs to sponsor"
      />
      ...
      <AnimatePresence>
        {Object.keys(selectedPrograms).length > 0 && (
          <motion.div initial={{ opacity: 0, y: 12, height: 0 }} animate={{ opacity: 1, y: 0, height: "auto" }} exit={{ opacity: 0, y: -8, height: 0 }}
            ...
            <h3 className="text-sm font-black text-gray-900 mb-4 flex items-center gap-2">🏢 Your Sponsored Programs</h3>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mt-2 pt-3 border-t border-gray-50 gap-3">
                      <div>
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide flex items-center gap-1 mb-1">
                          <Users className="w-3 h-3" /> Number of {prog.unit}s:
                        </label>
